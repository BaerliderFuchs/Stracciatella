...
public class Tap extends Block implements IWaterLoggable {
    ...
    public static final VoxelShape SHAPE_N = Stream.of(
            Block.makeCuboidShape(6, 7, 12, 7, 9, 16),
            Block.makeCuboidShape(9, 7, 12, 10, 9, 16),
            Block.makeCuboidShape(6, 6, 12, 10, 7, 16)
    ).reduce((v1, v2) -> {
        return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);
    }).get();

    public static final VoxelShape SHAPE_S = Stream.of(
            Block.makeCuboidShape(9, 7, 0, 10, 9, 4),
            Block.makeCuboidShape(6, 7, 0, 7, 9, 4),
            Block.makeCuboidShape(6, 6, 0, 10, 7, 4)
    ).reduce((v1, v2) -> {
        return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);
    }).get();

    public static final VoxelShape SHAPE_E = Stream.of(
            Block.makeCuboidShape(0, 7, 6, 4, 9, 7),
            Block.makeCuboidShape(0, 7, 9, 4, 9, 10),
            Block.makeCuboidShape(0, 6, 6, 4, 7, 10)
    ).reduce((v1, v2) -> {
        return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);
    }).get();
    ...
    public static final VoxelShape SHAPE_W = Stream.of(
            Block.makeCuboidShape(12, 7, 9, 16, 9, 10),
            Block.makeCuboidShape(12, 7, 6, 16, 9, 7),
            Block.makeCuboidShape(12, 6, 6, 16, 7, 10)
    ).reduce((v1, v2) -> {
        return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);
    }).get();

    public Tap(Properties properties) {
        super(properties);
        this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(WATERLOGGED, Boolean.valueOf(false)).with(RUNNING, false));
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult blockRayTraceResult) {
        if (!worldIn.isRemote()) {
            playerIn.sendMessage(new StringTextComponent("RUNNING: " + state.get(RUNNING)));
            worldIn.setBlockState(pos, (BlockState)state.with(RUNNING, !state.get(RUNNING)));
            playerIn.sendMessage(new StringTextComponent("RUNNING: " + state.get(RUNNING)));
        }
